"""Replay TASK-012 candidate evidence against one configured EVM provider."""
...
from scan_tool.application.task_012_replay import run_task_012_replay
...
async def async_main() -> int:
    ...
    try:
        ...
        output_root = resolve_output_root(
            Path(".scan/live-provider-smoke/task-012-replay"),
            REPOSITORY_ROOT,
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            report = await run_task_012_replay(
                role=role,
                endpoint=endpoint,
        ...
        json.dumps(
            {
                ...
                "report": f"task-012-replay://{report_path.name}",
            }
        )
    )
