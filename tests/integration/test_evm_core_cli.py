"""TASK-012 EVM Core CLI persistence, partial, and resume tests."""
...
def test_evm_core_analyze_persists_and_show_renders_exact_result(
    tmp_path: Path,
...
def test_evm_core_incomplete_range_is_partial_and_preserves_candidate(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(tmp_path)
    fixture_id = "FX-EVM-TOKEN-001"
    replay = json.loads(_path(fixture_id, "raw-replay.json").read_text())
    replay["range_complete"] = False
    replay_path = tmp_path / "incomplete-range.json"
    ...
    assert "PARTIAL AN-FX-EVM-TOKEN-001" in result.stdout
    assert "first_token_transfer" in result.stdout
    assert "evidence_incomplete" in result.stderr


def test_evm_core_keyboard_interrupt_resumes_from_saved_replay(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(tmp_path)
    ...
    monkeypatch.setattr(
        "scan_tool.application.cli_runtime.analyze_evm_core_replay",
        interrupt,
    )
    ...
    assert "INTERRUPTED AN-FX-BASIC-EVM-001" in interrupted.stdout

    monkeypatch.setattr(
        "scan_tool.application.cli_runtime.analyze_evm_core_replay",
        analyze_evm_core_replay,
    )
    resumed = runner.invoke(app, ["resume", "AN-FX-BASIC-EVM-001"])

    assert resumed.exit_code == 0
    assert "COMPLETE AN-FX-BASIC-EVM-001" in resumed.stdout
