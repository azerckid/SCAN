...
- primary와 verify provider ID·role은 서로 달라야 한다.
- stored artifact는 repository package 내부 경로와 SHA-256으로 검증한다.
- mempool.space·Blockstream stored projection을 직접 파싱해 root·hop 값을
  각각 재유도하고 서로 동일해야 한다. PublicNode의 txid·vin outpoint·vout·
  block hash/time도 그 projection과 일치해야 한다.
...
- 사용자 구현 승인: 2026-07-31 일괄 승인 메시지
