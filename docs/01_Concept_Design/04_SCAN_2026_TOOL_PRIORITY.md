...
| Challenge Pack | Candidate 1, 10문항 | 규모·모호성·장애 stress coverage. 현재 빈도·점수에는 미합산 |
| 데이터 소스 등록부 | Draft, EVM 핵심 소스 검증 중 | 외부 의존성과 재시도 위험 |
| 공통 fixture 스키마 | Confirmed 0.1 | 검증 입력·정답·증거 연결 계약 |
| 오픈소스 포렌식 사전조사 | Draft 1 · Initial Survey | 기능별 재사용·직접 구현 Gate |
...
Challenge Pack은 기본 기능의 필요성을 새로 주장하기보다 실제 데이터 규모,
불완전 단서, 추적 회피, 공급자 장애와 제한 시간에서 도구가 유지되는지
검증한다. 실제 fixture로 승격되기 전에는 이 문서의 필수·조건부 빈도와
우선순위 점수에 합산하지 않는다.
