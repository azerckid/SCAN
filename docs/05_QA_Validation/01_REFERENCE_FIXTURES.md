...
replay, negative oracle 16개와 독립 Verifier의 7개 requirement 두 번
재계산까지 확보한 `후보`다. 별도 승격 판단 전에는 `검증 중`이나
...
13. TASK-013 독립 Verifier가 raw replay에서 세 candidate의 필수 facts와
    7개 requirement를 두 번 재계산했다. UI·Context Receipt와 별도 승격
    판단이 남아 package는 `candidate`다.
