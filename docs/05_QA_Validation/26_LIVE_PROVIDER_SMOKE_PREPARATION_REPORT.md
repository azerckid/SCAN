...
> Status: Pre-event Smoke Executed · Common Capabilities Complete · Overall Partial

## 1. 목적과 판정

PR #41의 Live Provider Gate를 실제 실행할 최소 runner로 준비하고, primary와
verify endpoint를 로컬에 구성한 상태에서도 공식 Rules가 `unclear`이면
기본 실행이 열리지 않는지 검증한 뒤, 사용자 승인을 받은 대회 전 공급자
준비 점검을 실행했다.

**판정: 공통 capability와 primary trace는 성공했다. 독립 trace·rate
behavior·credential 회전이 남아 전체 Provider Gate는 partial이다.**
...
| live EVM calls | 13 read-only (primary 7, verify 6) |
| live LLM calls | 0 |

secret 값은 저장소·검증 출력·문서에 기록하지 않았다. 다만 설정 과정에서
두 credential이 대화에 노출됐다. 사용자는 pre-event smoke 실행 후 직접
회전하기로 했으며, 대회 사용과 후속 지속 호출 전 보안 Gate를 다시 확인한다.
...
operator-approved pre-event primary: complete, 7/7
operator-approved pre-event verify: complete, 6/6
common decoded summaries: 6/6 match
primary debug_traceTransaction: success
artifact configured-secret pattern: not found
primary methods:
  eth_chainId
...
1. 대회 live mode 전 공식 Rules에서 API·AI mode를 확인해야 한다.
2. 노출된 primary·verify credential을 회전하고 새 값을 로컬에서 직접 구성해야 한다.
3. trace-dependent `EVM-TOKEN-002`를 위해 독립 trace 역할을 정해야 한다.
4. rate limit·timeout·unsupported 반례를 확인해야 한다.
5. 네 candidate fixture의 provider별 raw replay·provenance를 연결해야 한다.
...
| 기준 | 판정 | 근거 |
|:---|:---:|:---|
| Functionality | Partial | 공통 RPC 6/6 일치·primary trace 성공, 독립 trace·반례 미실행 |
