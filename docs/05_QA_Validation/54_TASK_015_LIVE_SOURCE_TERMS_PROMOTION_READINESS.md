...
> Status: Applied · 3 Confirmed · LABEL Replacement Verifying · Common-funder Candidate
...
- label fixture의 기존 OpenRAIL selected row는 exact license를 특정할 수
  없어 scoring·provenance dependency에서 제거했다. replacement는 official
  historical action·MIT config·fixed-block ENS이며 별도 Promotion Review
  전까지 `verifying`을 유지한다.
...
|:---|:---:|:---|:---|:---|:---|
| `FX-OSINT-LABEL-CONFLICT-001` | verifying | official historical action projection·MIT config·fixed-block ENS | replacement replay·30 oracle·Verifier·analyzer canonical hash·OpenRAIL dependency 제거 | 별도 Promotion Review | MIGRATED / REVIEW PENDING |
...
7. [x] 새 subject의 replay·oracle·Verifier·analyzer hash를 재검증하고
...
- `scripts/verify.py`: **542 tests PASS**
...
- **QA_Validation**: [LABEL Migration Receipt](./58_TASK_015_LABEL_FIXTURE_MIGRATION_RECEIPT.md) - replacement replay·oracle·Verifier·analyzer 재검증
