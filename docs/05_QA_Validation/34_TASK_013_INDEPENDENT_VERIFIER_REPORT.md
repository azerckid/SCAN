...
> Status: Passed · 3 Candidates · 7 Requirements · Fixture Promotion Pending

## 1. 목적과 판정

TASK-013의 ERC-721, ERC-1155, EIP-1967 candidate가 가진 raw replay를
expected와 분리된 계산 경로로 다시 해석한다. Verifier는 raw 계산을 먼저
끝낸 뒤 expected projection과 requirement→evidence 연결을 대조한다.

**판정: 세 candidate의 필수 사실과 7개 requirement가 두 번의 독립 실행에서
모두 일치했다. fixture는 승격 판단 전까지 계속 `candidate`다.**
...
| Fixture | 필수 계산 | Requirement | Fact SHA-256 | 결과 |
|:---|:---|---:|:---|:---:|
| `FX-EVM-NFT-721-001` | Transfer, ApprovalForAll, Approval reset, token `9110` | 2 | `a2b879fb7aa9f157168b349875decff86d3a1d685792332c9eff1af8ca0e5e74` | pass |
| `FX-EVM-NFT-1155-001` | Single, Batch arrays, approval true→false | 2 | `0caf8a09994abbff71cb4afddf9bb6e11fa5411ef0870c27d1a92ea324aade91` | pass |
| `FX-EVM-PROXY-001` | upgrade event, adjacent implementation, admin zero | 3 | `f0683ef2167e2e7799891a66e0b065300842fd46de5d63b84ebe440ee2f58d93` | pass |
| **합계** | raw-first exact 계산 | **7** | 두 번 동일 | **pass** |
...
- `tests/unit/test_task_013_independent_verifier.py`
  - expected drift, raw topic, truncated ABI, event/state 충돌, evidence 누락
- `scripts/verify.py`
...
| 기준 | 판정 |
|:---|:---|
| Functionality | 세 candidate·7 requirements의 raw-first exact 재계산 통과 |
