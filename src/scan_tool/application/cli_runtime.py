...
from scan_tool.domain.defi_lending import DefiLendingReplay
        ...
        *,
        ...
        if isinstance(document, IntelContextAnalysisRequest):
            replay_body, checkpoint_id, resumed = self._load_replay(
                ...
                replay_body,
                expected_replay_sha256,
                stage=INTEL_CONTEXT_REPLAY_STAGE,
                replay_model=IntelSourceReplay,
                analysis_label="Intel Context",
            )
            return analyze_intel_context_replay(
                document,
                replay_body,
                resumed=resumed,
                checkpoint_id=checkpoint_id,
        ...
        if isinstance(document, BitcoinUtxoAnalysisRequest):
            if replay_path is None:
                raise AnalysisUnavailable(
                    "Bitcoin UTXO analysis requires --evidence RAW_REPLAY.json so sibling "
                    "content-addressed provider artifacts can be verified."
                )
            package_dir = replay_path.parent
            replay_body, checkpoint_id, resumed = self._load_replay(
                document.analysis_id,
                ...
                replay_body,
                expected_replay_sha256,
                stage=BITCOIN_UTXO_REPLAY_STAGE,
                replay_model=BitcoinUtxoReplay,
                analysis_label="Bitcoin UTXO",
            )
            return analyze_bitcoin_utxo_replay(
                document,
                replay_body,
                package_dir=package_dir,
                resumed=resumed,
        ...
        if isinstance(document, CexClusterAnalysisRequest):
            if replay_path is None:
                raise AnalysisUnavailable(
                    "CEX Cluster analysis requires --evidence RAW_REPLAY.json with a "
                    "file path on every invocation because the replay references sibling "
                    "content-addressed artifacts. It does not support Evidence Worker "
                    "byte-only replay or checkpoint-only resume."
                )
            ...
            replay_body, checkpoint_id, resumed = self._load_replay(
                ...
                replay_body,
                expected_replay_sha256,
                stage=CEX_CLUSTER_REPLAY_STAGE,
                replay_model=CexClusterReplay,
                analysis_label="CEX Cluster",
            )
            return analyze_cex_cluster_replay(
                document,
                replay_body,
                package_dir=package_dir,
                resumed=resumed,
        ...
        if isinstance(document, DefiLendingAnalysisRequest):
            if replay_path is None:
                raise AnalysisUnavailable(
                    "DeFi Lending analysis requires --evidence RAW_REPLAY.json with a "
                    "file path on every invocation because the replay references sibling "
                    "content-addressed artifacts. It does not support Evidence Worker "
                    "byte-only replay or checkpoint-only resume."
                )
            ...
            replay_body, checkpoint_id, resumed = self._load_replay(
                ...
                replay_body,
                expected_replay_sha256,
                stage=DEFI_LENDING_REPLAY_STAGE,
                replay_model=DefiLendingReplay,
                analysis_label="DeFi Lending",
            )
